...
// Modify this variable to change the number of threads
#define MAX_THREADS 8

uint32_t read_hashes_from_file(FILE* bpkg_file, char hashes[][SHA256_HEXLEN + 1], uint32_t nhashes);
struct merkle_tree_node* create_non_leaf_nodes(char hashes[][SHA256_HEXLEN + 1], uint32_t count);
...
struct bpkg_obj* bpkg_load(const char* path) {
    ...
    while (fgets(file_line, sizeof(file_line), bpkg_file) != NULL){
        ...
        if (strcmp(left, "ident") == 0){
            ...
            char hashes[1024][SHA256_HEXLEN+1];
            uint32_t hash_count = read_hashes_from_file(bpkg_file, hashes, obj->nhashes);
            struct merkle_tree_node* nodes = create_non_leaf_nodes(hashes, hash_count);
            obj->merkle_tree->root = &nodes[0];
...
uint32_t read_hashes_from_file(FILE* bpkg_file, char hashes[][SHA256_HEXLEN + 1], uint32_t nhashes){
    uint32_t count = 0;
    while (count < nhashes && fscanf(bpkg_file, "%64s", hashes[count]) == 1) {
        count++;
    }
...
void* create_non_leaf_nodes_task(void* arg) {
    struct thread_data* data = (struct thread_data*)arg;
    struct merkle_tree_node* nodes = data->nodes;
    char (*hashes)[SHA256_HEXLEN + 1] = data->hashes;
    ...
    for (uint32_t i = start; i < end; i++) {
        ...
        nodes[i].right = NULL;
        nodes[i].is_leaf = 0;
        nodes[i].offset_in_file = 0;
        nodes[i].size_in_file = 0;
        strncpy(nodes[i].expected_hash, hashes[i], SHA256_HEXLEN);
        memset(nodes[i].computed_hash, 0, SHA256_HEXLEN);  // Initialize computed_hash to zero

        uint32_t left_index = 2 * i + 1;
        uint32_t right_index = 2 * i + 2;
...
struct merkle_tree_node* create_non_leaf_nodes(char hashes[][SHA256_HEXLEN + 1], uint32_t count) {
    struct merkle_tree_node* nodes = (struct merkle_tree_node*)malloc(count * sizeof(struct merkle_tree_node));
    if (nodes == NULL) {
        perror("create_non_leaf_nodes: Failed to allocate memory");
        return NULL;
    }

    pthread_t threads[MAX_THREADS];
    struct thread_data thread_args[MAX_THREADS];
    uint32_t step = (count + MAX_THREADS - 1) / MAX_THREADS;  // Divide work among threads
    ...
    for (int i = 0; i < MAX_THREADS; i++) {
        ...
        if (thread_args[i].end > count) {
            thread_args[i].end = count;
        }
        thread_args[i].count = count;
        pthread_create(&threads[i], NULL, create_non_leaf_nodes_task, (void*)&thread_args[i]);
    }
